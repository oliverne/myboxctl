...
2. 각 component를 exact resolve한다.
3. folder면 다음 component로 진행한다.
4. file이면 즉시 conflict를 반환한다.
5. absent면 현재 parentId로 `createFolder`를 한 번 호출한다.
6. 409 또는 응답 유실 가능 오류면 같은 exact path를 재조회한다.
7. created/reconciled folder ID를 다음 parent로 사용한다.

같은 명령 실행 중 확인한 component는 메모리에 유지한다. process를 넘는 cache나 state DB는
추가하지 않는다.
