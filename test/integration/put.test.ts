...
type CliOutput = {
  ok: boolean;
  action?: string;
  data?: { path?: string; sizeBytes?: number; reason?: string };
...
describeIntegration("MYBOX upload metadata acceptance", () => {
  beforeAll(async () => {
  ...
  test("runs the metadata policy flow and reuses mkdir/upload", async () => {
    ...
    const same = await runCli(["upload", localPath, filePath, "--json"]);
    ...
    const forced = await runCli(["upload", localPath, filePath, "--force", "--json"]);
    ...
    const missingParent = await runCli(["upload", localPath, nestedFilePath, "--json"]);
    ...
    const mkdir = await runCli(["upload", localPath, nestedFilePath, "--mkdir", "--json"]);
